...
export default function GroupCard({ group: g }: { group: Group }) {
  ...
  return (
    ...
    >
      <div className="mb-3 flex items-start justify-between">
        <h3
          className={`text-lg font-semibold ${isPending ? "text-slate-500" : "text-slate-900"}`}
        >
          {g.name}
        </h3>
        {isPending && (
          <span className="rounded-full bg-[#ff0080]/10 px-2.5 py-0.5 text-xs font-medium text-[#ff0080]">
            Pending
          </span>
        )}
        {g.myRole === "owner" && !isPending && (
          <span className="rounded-full bg-[#009de5]/10 px-2.5 py-0.5 text-xs font-medium text-[#009de5]">
            Owner
          </span>
        )}
      </div>
