    ...
    <Modal title="Join Group" onClose={onClose}>
      <form action={formAction}>
        ...
        <input
          ...
          className="w-full rounded-xl border border-slate-200 bg-slate-50 px-5 py-3.5 text-lg text-slate-900 placeholder-slate-400 transition-colors focus:border-[#009de5]/40 focus:outline-none focus:ring-2 focus:ring-[#009de5]/20"
        />
        {hints.map((h) => (
        ...
        {state?.error && !dismissed && (
          <p className="mt-2 text-base text-red-500">{state.error}</p>
        )}
